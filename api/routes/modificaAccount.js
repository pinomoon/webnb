...
async function modifica(req, res, next) {
  ...
  try {
    await withTransaction(db, async() => {
      ...
      results = await db.query('SELECT nome,cognome,tipo,data_di_nascita,indirizzo,sesso,citta,cap,cellulare,numero_carta,scadenza,cvc  FROM utente,carta_credito WHERE utente.email=carta_credito.email AND id_utente = ?', [
          req.body.id_utente
      ])
          .catch(err => {
              throw err;
               ...
               , 
                     ...
                     [
                      ...
                      req.body.cellulare
                        ...
                        results= await db.query("UPDATE carta_credito SET numero_carta=?,scadenza=?,cvc=? WHERE carta_credito.email=req.body.email",
                        [
